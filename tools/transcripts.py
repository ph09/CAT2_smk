...
class Transcript(object):
    ...
    def get_mrna(self, seq_dict):
        ...
        if self.stop > len(sequence) + 1:
            raise ValueError(
                f"Transcript {self.name} on chromosome {self.chromosome} has stop coordinate "
                f"{self.stop} which exceeds sequence length {len(sequence)}. "
                f"Transcript span: {self.start}-{self.stop}. "
                f"This suggests coordinate mismatch between GFF3 and FASTA files."
            )
        s = []
        for e in self.exon_intervals:
    ...
    def get_start_intervals(self):
        ...
        if self.cds_size < 3:
            return []
        positions = [self.cds_coordinate_to_chromosome(x) for x in range(3)]
        ...
        if sum(len(x) for x in intervals) != 3:
            return []
        c = 0
        for i in intervals:
    ...
    def get_stop_intervals(self):
        ...
        if self.cds_size < 3:
            return []
        positions = [self.cds_coordinate_to_chromosome(x) for x in range(self.cds_size - 3, self.cds_size)]
        if any(p is None for p in positions):
            return []
        positions = sorted(positions)
        merged_intervals = list(find_intervals(positions))
    ...
    for gp in gp_list:
        for t in gene_pred_iterator(gp):
            ...
            if original_name in r:
                ...
                # For overlapping isoforms, keep same gene ID (don't add _cp suffix to name2)
            r[t.name] = t
    return r
