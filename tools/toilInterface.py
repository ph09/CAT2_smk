...
def load_fasta_from_filestore(job, fasta_file_ids, prefix='genome', upper=False):
    """
    Convenience function that will load a fasta from the fileStore and return the local path to it.
    Uses pysam with a .fai index.
    :param job: current job.
    :param fasta_file_ids: list of fileStore file ID for the fasta and fai file.
    :param prefix: local file path prefix
    :param upper: force all entries to upper case
    :return: FastaDict record pointing to the file.
    """
    fasta_local_path = '{}.fasta'.format(prefix)
    if len(fasta_file_ids) == 3:
        fasta_file_id, _legacy, fai_file_id = fasta_file_ids
    else:
...
def write_fasta_to_filestore(toil, fasta_local_path):
    """
    Convenience function that loads a fasta and its associated .fai index into the fileStore.
    :param toil: Toil context manager
    :param fasta_local_path: Path to local fasta to load.
    :return: List of fileStore IDs for fasta and fasta.fai
    """
