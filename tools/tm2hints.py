...
def is_fuzzy_intron(intron, tm_psl, ref_starts, fuzz_distance=12):
    ...
    # target_coordinate_to_query returns None when the position is outside
    # the alignment span or falls in an alignment gap. In that case we cannot
    # match the intron boundaries to any reference intron boundary in query
    # coordinate space — treat the intron as non-fuzzy (no reference match)
    # rather than crashing the whole hints job. Downstream this just means
    # Augustus does not get a "matched intron" hint for this intron.
    if q_gap_start is None or q_gap_stop is None:
        return False
