...
@memoize
def extract_ancestor_genomes(hal, target_genomes):
    """
    Return internal (ancestral) node names on the HAL subtree spanning target_genomes.
    :param hal: HAL file.
    :param target_genomes: Leaf genomes whose spanning subtree defines which ancestors to include.
    :return: tuple of ancestral genome names (internal nodes only).
    """
    t = get_tree(hal)
    if not target_genomes:
        return tuple()
    mrca = t.get_common_ancestor(list(target_genomes))
    return tuple(
        node.name for node in mrca.traverse()
        if not node.is_leaf()
    )


@memoize
def list_hal_genomes(hal):
    """All genome names (leaves and ancestors) present in the HAL."""
    t = get_tree(hal)
    return tuple(node.name for node in t.traverse())
